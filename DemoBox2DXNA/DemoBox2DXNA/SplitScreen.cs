...
namespace DemoBox2DXNA
{
    class SplitScreen: IState
    {
        ...
        public SplitScreen(IPlay iPlay, Game game): base(iPlay, game)
        {
            ID = STATEGAME.SLITSCREEN;
        }
        ...
        public override void Render(GameTime gameTime, Microsoft.Xna.Framework.Graphics.SpriteBatch _SpriteBatch)
        {
            ...
            String msg = "This is Splitting BOX2DDEMO!";
            Vector2 textSize = spriteFont.MeasureString(msg);
            spriteBatch.DrawString(spriteFont, msg, new Vector2(400 - textSize.X/2, 20), Color.RoyalBlue);
            
            msg = "Press Q W E R T Y :)) to add polygon, F to add floor, P to clear";
            textSize = spriteFont.MeasureString(msg);
            spriteBatch.DrawString(spriteFont, msg, new Vector2(400 - textSize.X / 2, 40), Color.Red);

            msg = "Then use mouse to split them! Enjoy it! :)";
            textSize = spriteFont.MeasureString(msg);
            spriteBatch.DrawString(spriteFont, msg, new Vector2(400 - textSize.X / 2, 60), Color.Green);
            if (isDrawing)
            {
        ...
        void Resize(int w, int h)
        {
            ...
            tw = Game.GraphicsDevice.Viewport.Width;
            th = Game.GraphicsDevice.Viewport.Height;
            int x = Game.GraphicsDevice.Viewport.X;
            int y = Game.GraphicsDevice.Viewport.Y;
